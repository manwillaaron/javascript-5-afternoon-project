...
function Movie(name, genre, rating) {
  ...
  Movie.prototype.changeRating = function(num){
    return Movie.rating = num
    // let index = Movie.findIndex(movie => movie.name === movie)
    // return Movie
}
...
// It should take in three parameters: id (a number), title (a string) and rating (a number). Use these parameters to create a new object and add it 
// to the savedPosts array. Make sure to name the properties the same as described previously (id, title, rating).
...
//and the second will be the new rating (a number). Use the id to find the matching object in the savedPosts array. Once you find the matching object, 
...
User.prototype.changePostRating = function(id, num){
  let index = this.savedPosts.findIndex(post => post.id === id)
    return this.savedPosts[index].rating = num
